...
flowchart TD
  ...
  B_SEARCH -->|2. Gửi request tìm kiếm| C_QUERY
  C_QUERY -->|3. Truy vấn Catalog| E_CAT
  C_QUERY -.->|A1. Không có kết quả| B_SEARCH
  C_QUERY -->|4. Trả danh sách| B_SEARCH
  
  A_CUS -->|5. Click chọn một mục| B_SEARCH
  B_SEARCH -->|6. Điều hướng xem chi tiết| B_DETAIL
  B_DETAIL -->|7. Yêu cầu tải chi tiết| C_FETCH
  C_FETCH -->|8. Đọc dữ liệu Cơ sở| E_CLINIC
  C_FETCH -->|Đọc dữ liệu Bác sĩ| E_DOC
  C_FETCH -->|Trích xuất tiêu sử| E_SPEC
  C_FETCH -->|Đọc dữ liệu Dịch vụ| E_SERV
  C_FETCH -->|9. Lọc slot trống| E_SLOT
  C_FETCH -->|10. Render giao diện chi tiết| B_DETAIL
  
  C_FETCH -.->|A2. Báo hết Slot & reload| B_DETAIL
```
...
*Sơ đồ mô tả chu trình Actor tương tác với Boundary Trang Wishlist, Control xử lý nghiệp vụ thêm/xóa và cập nhật dữ liệu vào Entity Wishlist và WishlistItem; các luồng lỗi (chưa đăng nhập) được phản ánh bằng đường đứt nét trở về Boundary.*
...
*Sơ đồ mô tả Actor tương tác với Boundary Trang Giỏ hàng, Control quản lý Cart kiểm tra AvailabilitySlot và tạo/cập nhật CartItem; nhánh lỗi xung đột slot được biểu diễn bằng đường đứt nét.*
...
*Sơ đồ mô tả luồng Khách hàng xác nhận đơn, Control khởi tạo BookingOrder, khoá slot và xử lý thanh toán qua Cổng API; các nhánh ngoại lệ (hết slot, nhiều cơ sở, thất bại) đều trả về Boundary bằng đường đứt nét.*
...
flowchart TD
  ...
  A_CUS -->|1. Xác nhận giỏ| B_CHK_UI
  B_CHK_UI -->|2. Khởi tạo đơn| C_CHK
  C_CHK -->|3. Kiểm tra/Lock Slot| E_SLOT
  C_CHK -->|Đọc giỏ| E_CART_ITEM
  C_CHK -->|Tạo danh sách item| E_ORDER_ITEM
  C_CHK -->|Tạo BookingOrder (PendingPayment)| E_ORDER
  C_CHK -.->|A1. Hết Slot| B_CHK_UI
  C_CHK -.->|A2. Nhiều cơ sở y tế| B_CHK_UI
  
  A_CUS -->|4. Chọn phương thức & thanh toán| B_CHK_UI
  B_CHK_UI -->|5. Gửi yêu cầu thanh toán| C_PAY
  C_PAY -->|6. Gọi API thanh toán| B_GATEWAY
  B_GATEWAY <--> A_PAYG
  
  B_GATEWAY -->|7. Nhận kết quả| C_PAY
  C_PAY -.->|A3. Thanh toán thất bại/hủy| B_CHK_UI
  C_PAY -->|Ghi nhận giao dịch| E_TRANS
  C_PAY -->|Cập nhật Paid/Confirmed| E_ORDER
  C_PAY -->|8. Gọi UC-12| C_CALL_12
  C_CALL_12 -.->|A4. Lỗi QR/Thông báo: ghi nhận & retry| E_ORDER
```

**Hình 3.5 – RB-UC04: Đặt lịch & Thanh toán**

Nội dung Hình 3.5: Khách hàng checkout (Boundary). Control khởi tạo đơn kiểm tra/lock slot (`AvailabilitySlot`), đọc `CartItem`, tạo `BookingItem` và `BookingOrder` (PendingPayment); có nhánh ngoại lệ khi hết slot hoặc giỏ có mục thuộc nhiều cơ sở. Control thanh toán giao tiếp với gateway/cổng thanh toán, ghi `PaymentTransaction`, cập nhật trạng thái đơn; có nhánh thanh toán thất bại/hủy để người dùng retry. Khi đơn đủ điều kiện, hệ thống gọi UC-12; nếu phát hành QR/ICS hoặc gửi thông báo lỗi thì đơn vẫn hợp lệ và hệ thống ghi nhận để retry.
...
*Sơ đồ mô tả Khách hàng kích hoạt hủy/hoàn từ Boundary, Control kiểm tra CancelPolicy và cập nhật BookingOrder, Control hoàn tiền tạo RefundRequest và kết nối gateway; nhánh từ chối và lỗi được thể hiện bằng đường đứt nét.*
...
*Sơ đồ mô tả hai Actor (Khách hàng và NV Kiểm duyệt) thao tác trên hai Boundary riêng biệt; Control phân luồng lưu Review chờ duyệt và cập nhật ReviewModeration, với các nhánh từ chối lỗi hợp lệ dữ liệu bằng đường đứt nét.*
...
flowchart TD
  ...
  A_CUS -->|1. Nhập nội dung & gửi review| B_REV
  B_REV -->|2. Submit| C_SEND
  C_SEND -.->|A1. Chưa đăng nhập| B_REV
  C_SEND -.->|A2. Nội dung/rating không hợp lệ| B_REV
  C_SEND -->|3. Gắn khóa Bác sĩ| E_DOC
  C_SEND -->|hoặc khóa Cơ sở| E_CLINIC
  C_SEND -->|4. Lưu Review (Pending)| E_REV
  C_SEND -.->|A3. Cảnh báo spam| B_REV
  
  A_MOD -->|5. Mở danh sách chờ duyệt| B_MOD
  B_MOD -->|6. Duyệt| C_MOD
  C_MOD -.->|A4. Review không còn hợp lệ| B_MOD
  C_MOD -->|7. Tạo log kiểm duyệt| E_REV_MOD
  C_MOD -->|8. Update trạng thái Review| E_REV
```

**Hình 3.7 – RB-UC07-08: Đánh giá & Kiểm duyệt**

Nội dung Hình 3.7: Khách hàng gửi đánh giá qua Form Đánh giá (Boundary). Control lưu & chờ duyệt kiểm tra điều kiện (đăng nhập, hợp lệ dữ liệu), liên kết `Review` tới `Doctor` hoặc `ProviderClinic` và lưu review ở trạng thái chờ duyệt; có nhánh cảnh báo/từ chối khi spam hoặc dữ liệu không hợp lệ. Nhân viên kiểm duyệt thao tác trên Trang Kiểm duyệt (Boundary); Control duyệt trạng thái cập nhật kết quả, ghi `ReviewModeration`, đồng thời xử lý trường hợp review không còn hợp lệ.
...
*Sơ đồ mô tả Actor Biên tập viên thao tác trên Boundary Trang Soạn thảo, Control đăng bài lưu Draft và cập nhật trạng thái Published cho EditorialPost; nhánh lỗi thiếu thông tin bắt buộc được thể hiện bằng đường đứt nét.*
...
*Sơ đồ mô tả hai tuyến dữ liệu đối xứng: tuyến Import từ NV Cơ sở y tế qua Boundary Nhập → Control Xác thực và Hợp nhất → Catalog; tuyến Export từ Actor Đối tác qua Boundary API XML → Control Sinh XML → trả dữ liệu; nhánh lỗi định dạng được biểu diễn bằng đường đứt nét.*
...
*Sơ đồ mô tả UC-12 như một Background Service: Control kích hoạt sự kiện từ các UC trước, giao tiếp với dịch vụ QR/ICS qua Boundary File và ghi AppointmentTicket, sau đó bắn thông báo qua Boundary Push và ghi Notification; nhánh lỗi/retry biểu diễn bằng đường đứt nét.*
